...
public class TherapistDirectoryService {
    ...
    public TherapistDirectoryService(TherapistProfileRepository therapistProfileRepository) {
        this.therapistProfileRepository = therapistProfileRepository;
    }

    public List<TherapistDirectoryResponse> listAvailableTherapists(){
        return therapistProfileRepository.findByAcceptingClientsTrue()
                .stream()
                .map(TherapistDirectoryResponse::from)
                .toList();
    }

    public TherapistDirectoryResponse getAvailableTherapist(UUID userId){
        TherapistProfile profile = therapistProfileRepository.findByUserIdAndAcceptingClientsTrue(userId)
